...
Function arguments:\\
\begin{description}
    \item[\texttt{sampleFile}:] (string) sample filename, needs to be a
    fastq file
    \item[\texttt{libFile}:] (string) library filename, needs to be a fasta
    or fastq file.
...
\pkgname{} provides two approches for the user to defined their own
...
\begin{align*}
    ...
    \end{cases}
\end{align*}
This definition assumes that suquences before position $j$ has influence on the
error rate. The option can be actived by providing a dataframe to the argument
\texttt{tMat}. The first column of this dataframe is a vector of sequences 
$[\r_{j-n},\ldots,\r_j]$ and the second column is the corresponding 
value of $f(\epsilon_j)_{[\r_{j-n},\ldots,\r_j]}$.  If a sequence is not 
in the dataframe the default values $\frac{\epsilon_j}{3}$ are used.
\end{frame}
...
comstomizeP <- function(max_pen, prob, pen_val)
{
