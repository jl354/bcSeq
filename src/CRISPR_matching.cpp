...
// sort by barcode
bool by_bc(const res_t& x, const res_t& y)
{
  return get<1>(x) < get<1>(y);
}

// sort by pr
bool by_pr(const res_t& x, const res_t& y)
{
...
void clean(Trie& trie,
           vector<res_t>::iterator start,
           vector<res_t>& results)
{
  ...
  for( auto i = start; i < results.end(); i++ )
  {
    ...
    {
      keep.push_back(*max_element(last,
                                  i + 1,
                                  by_pr));
      last = i + 1;
    }
...
*/
auto extract(vector<res_t>::iterator start,
             vector<res_t>::iterator end,
             vector<res_t>& keep,
             double m,
             array<double, 4>& pen,
             Function tForm)
...
void user_alignment(Trie& trie, vector<string>& sequence, vector<string>& phredScore,
