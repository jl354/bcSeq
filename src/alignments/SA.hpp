...
public:
  ...
  template<typename T>
  SA(T* x, tag_t t)
  : alignment(static_cast<void*>(x), deleter_t(t))
  , tag(t)
  {}
  ...
  SA(SA&& o)
  : alignment(std::move(o.alignment))
  , tag(o.tag)
  {}
};
...
template<>
struct get_tag<Match>
{
  tag_t tag = tag_t::Match;
};

template<>
struct get_tag<Insertion>
{
  tag_t tag = tag_t::Insertion;
};

template<>
struct get_tag<Deletion>
{
  tag_t tag = tag_t::Deletion;
};

template<>
struct get_tag<I_Extension>
{
  tag_t tag = tag_t::I_Extension;
};


template<>
struct get_tag<D_Extension>
{
  tag_t tag = tag_t::D_Extension;
};
