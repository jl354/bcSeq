...
public:
    ...
    Trie(double gap, double ext, double max)
        : root(new Node())
        , _penalties{{gap, ext, gap, ext}}
        , pen_max(max)
    {
    }

    Trie(double gap0, double ext0,
        double gap1, double ext1, double max)
        : root(new Node())
        , _penalties{{gap0, ext0, gap1, ext1}}
        , pen_max(max)
    {
    }
    ...
    auto bounded() -> bool
    {
        return pen_max != 0 && (_penalties[0] || _penalties[1] ||
                                _penalties[2] || _penalties[3]);
    }
